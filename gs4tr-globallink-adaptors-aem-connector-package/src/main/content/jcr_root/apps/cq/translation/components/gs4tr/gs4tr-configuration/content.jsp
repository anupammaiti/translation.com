...
<div>
    ...
    <ul style="float: left; margin: 0px 0px 0px 0px;">
        ...
        <li>
            <div class="li-bullet">
                <strong><%= i18n.get("Short") %>:</strong>
                <%= xssAPI.encodeForHTML(projectShortCode) %>
            </div>
        </li>
		<li>
            <div class="li-bullet">
                <strong><%= i18n.get("Default Classifier") %>:</strong>
     ...
     <div> <h3><%= i18n.get("Classifier Configuration") %></h3></div>
	 <table border = "0">
        <tr><td><b>MIME type</b></td> <td><b>Classifier</b></td></tr>

         <%
			String[] customClassifierArray = properties.get("pdProjectCustomClassifier",String[].class);
 		if(customClassifierArray!=null){
            if(customClassifierArray.length>0){
                for(String mimeClassifier : customClassifierArray){
                 if(mimeClassifier.length()>0){
                      String[] classifier = mimeClassifier.split("/");
                      %><tr><td><%=classifier[0]%></td><td><%=classifier[1]%></td><%
                 }
        	}
           }
    	}
        ...
        try{
            ...
            while (items.hasNext()) {
                ...
                if (property == null) {
        		 ...
        		 node = property.adaptTo(Node.class);
        		String languageName = "";
        		String languageCode = "";
				String fuzzyWords= "";
        		String newWords= "";
